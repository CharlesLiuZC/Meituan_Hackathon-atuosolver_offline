...
def apply_runtime_overrides():
    ...
    if case_type == "low_willingness":
        overrides = {
            ...
            "backup_reallocation_budget_ms": 3500.0,
            "ilp_time_limit_seconds": 0.0,
            "min_backup_utility": 0.0,
            "max_extra_couriers_per_bundle": 8,
            "backup_fair_first": False,
        ...
        overrides = {
            "enable_multi_courier_output": True,
            "auto_strategy_budget_ms": 300.0,
            "local_search_budget_ms": 2200.0,
            "multi_primary_time_budget_ms": 0.0,
            "backup_time_budget_ms": 2000.0,
            "backup_reallocation_budget_ms": 800.0,
            "ilp_time_limit_seconds": 0.0,
            "min_backup_utility": 0.0,
            "max_extra_couriers_per_bundle": 6,
            "backup_fair_first": False,
            "_runtime_multi_cost_mode": "race",
        }
    else:
...
def base_orders(ctx):
    ...
    if CONFIG.get("_runtime_low_willingness", False):
        orders.extend([
            ("lw_high_w_primary", sorted(ctx.candidates, key=lambda c: (0 if c.task_count > 1 else 1, -c.willingness, candidate_penalty_cost(c), c.score))),
            ("lw_official_w_first", sorted(ctx.candidates, key=lambda c: (0 if c.task_count > 1 else 1, -c.willingness, official_penalty_cost(c) / max(1, c.task_count), c.score))),
            ("lw_score_low_w", sorted(ctx.candidates, key=lambda c: (0 if c.task_count > 1 else 1, -c.willingness, c.score, c.score_per_task))),
            ("lw_penalty_risk_adj", sorted(ctx.candidates, key=lambda c: (candidate_penalty_cost(c) / max(0.03, c.willingness + 0.03), candidate_penalty_cost(c), c.score))),
        ])
    if CONFIG.get("_runtime_scarce_couriers", False):
        orders.extend([
            ("sc_low_score_bundle", sorted(ctx.candidates, key=lambda c: (0 if c.task_count > 1 else 1, c.score, -c.willingness, candidate_penalty_cost(c)))),
            ("sc_willingness_bundle", sorted(ctx.candidates, key=lambda c: (0 if c.task_count > 1 else 1, -c.willingness, c.score, candidate_penalty_cost(c) / max(1, c.task_count)))),
        ])
    return orders
...
def _group_accept_prob(group):
    """Acceptance probability for a courier group (race model)."""
    fail = 1.0
    for c in group:
        fail *= 1.0 - min(1.0, max(0.0, c.willingness))
    return 1.0 - fail


def choose_probability_backups(candidates, selected, deadline_ms):
    """Batch backup allocation: precompute all utilities, sort, assign greedily."""
    ...
    # Iterative batch allocation: in each round, precompute marginal utility
    # for all (group, best_available_backup) pairs, sort globally, then assign
    # the best. Repeat until no positive-utility assignments remain.
    while _has_time(deadline_ms, 80.0) and _now_ms() - start_ms < CONFIG["backup_time_budget_ms"]:
        # Precompute: for each group, find the single best available backup
        candidates_pool = []  # (utility, primary, backup)
        for primary in selected:
            ...
            for backup in by_task_str.get(primary.task_str, ()):
                if id(backup) in selected_ids or backup.courier_id in used_couriers:
                    continue
                new_cost = multi_group_penalty([primary] + current + [backup])
                utility = current_cost - new_cost
                if utility > min_utility + EPS:
                    candidates_pool.append((utility, primary, backup))
                break  # only consider the best available per group

        if not candidates_pool:
            break

        # Pick the globally best assignment
        candidates_pool.sort(key=lambda x: -x[0])
        _, primary, backup = candidates_pool[0]
...
def optimize_primary_backup_swaps(selected, backup_map, total_task_count, deadline_ms):
    """Try swapping primary with each backup to find lower total penalty."""
    if not selected or not _has_time(deadline_ms, 100.0):
        return selected, backup_map

    current = list(selected)
    current_backups = {k: list(v) for k, v in backup_map.items()}
    current_eval = evaluate_with_backups(current, current_backups, total_task_count)

    improved = True
    while improved and _has_time(deadline_ms, 80.0):
        improved = False
        for gi in range(len(current)):
            if not _has_time(deadline_ms, 50.0):
                break
            primary = current[gi]
            group_backups = current_backups.get(id(primary), [])
            if not group_backups:
                continue

            for bi, backup in enumerate(group_backups):
                # Try: backup becomes primary, old primary becomes backup
                new_primary = backup
                new_group_backups = list(group_backups)
                new_group_backups[bi] = primary

                new_current = list(current)
                new_current[gi] = new_primary

                new_backups = {k: list(v) for k, v in current_backups.items()}
                new_backups[id(new_primary)] = new_group_backups
                # Remove old primary key if different
                old_key = id(primary)
                if old_key in new_backups and old_key != id(new_primary):
                    del new_backups[old_key]

                new_eval = evaluate_with_backups(new_current, new_backups, total_task_count)
                if is_better(new_eval, current_eval):
                    current = new_current
                    current_backups = new_backups
                    current_eval = new_eval
                    improved = True
                    break

    return current, current_backups
...
def improve_low_with_multi_options(candidates, task_to_idx, courier_to_idx, selected, deadline_ms):
    ...
    # Willingness-aware topology seeds: prioritize high-willingness primary couriers
    for seed, amplitude in (
        (3, 2.0), (7, 4.0), (19, 6.0), (37, 3.0), (53, 5.0),
        (11, 8.0), (23, 3.0), (41, 7.0), (67, 2.0), (89, 10.0),
    ):
...
def improve_scarce_with_multi_options(candidates, task_to_idx, courier_to_idx, selected, deadline_ms):
    ...
    if best_selected is None:
        best_selected = selected
        best_backups = choose_probability_backups(candidates, selected, deadline_ms)
    best_selected, best_backups = optimize_primary_backup_swaps(
        best_selected, best_backups, total_tasks, deadline_ms
    )
    return best_selected, best_backups
